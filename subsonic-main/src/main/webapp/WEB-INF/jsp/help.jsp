...
<c:choose>
    <c:when test="${model.buildDate != null}">
...
<c:choose>
    <c:when test="${model.localVersion != null}">
...
<table border="1" cellpadding="5" width="75%" rules="all">
    <tr><td><b><fmt:message key="help.version.title"/></b></td><td><c:out value="${versionString}"/></td></tr>
    <tr><td><b><fmt:message key="help.builddate.title"/></b></td><td><c:out value="${buildDateString}"/></td></tr>
...
<table cellpadding="2" style="white-space:nowrap;">
    <c:forEach items="${model.logEntries}" var="entry">
        <tr>
            ...
            <td><c:out value="${entry.level}"/></td><td><c:out value="${entry.category}"/></td><td><c:out value="${entry.message}"/></td>
        </tr>
    </c:forEach>
</table>
