    ...
    <card id="main" title="Subsonic" newcontext="false">
        ...
        <p><small>
            ...
            <c:when test="${empty model.players}">
                ...
                <c:otherwise>
                <b><a href="wapIndex.view">[<fmt:message key="common.home"/>]</a></b><br/>
                <b><a href="wapLoadPlaylist.view">[<fmt:message key="wap.playlist.load"/>]</a></b><br/>
                <c:set var="playlist" value="${model.players[0].playlist}"/>

                <c:if test="${not empty playlist.files}">
                <b><a href="wapPlaylist.view?clear">[<fmt:message key="wap.playlist.clear"/>]</a></b><br/>
        </small></p>
        <p><small>
            ...
            <c:forEach items="${playlist.files}" var="file" varStatus="loopStatus">
                ...
                ${isCurrent ? "<b>" : ""}
                <a href="wapPlaylist.view?skip=${loopStatus.count - 1}">${fn:escapeXml(file.title)}</a>
                ${isCurrent ? "</b>" : ""}
                <br/>
            </c:forEach>
            </c:if>
